...
TlsVectorEntry* ConstructTlsVector() {
  PlatformThreadLocalStorage::TLSKey key =
      NoBarrier_Load(&g_native_tls_key);
  if (key == PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES) {
    ...
    if (PlatformThreadLocalStorage::TLS_KEY_OUT_OF_INDEXES !=
      ...
      key = NoBarrier_Load(&g_native_tls_key);
    }
  }
...
void OnThreadExitInternal(TlsVectorEntry* tls_data) {
  ...
  {
    std::lock_guard<std::mutex> auto_lock(*GetTLSMetadataLock());
...
void* ThreadLocalStorage::Slot::Get() const {
  ...
  const TlsVectorState state = GetTlsVectorStateAndValue(
      NoBarrier_Load(&g_native_tls_key), &tls_data);
  DCHECK_NE(state, TlsVectorState::kDestroyed);
  if (!tls_data)
    return nullptr;
  DCHECK_NE(slot_, kInvalidSlotValue);
